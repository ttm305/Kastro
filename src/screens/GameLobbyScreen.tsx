...
import { getOrCreateLobby, getLobbyPlayers, setLobbyReady, leaveLobby, subscribeToLobby, type CosmeticItem } from '../lib/api'
...
export default function GameLobbyScreen({ onNavigate, lang, setLang, gameId }: Props) {
  ...
  // Tapping a player row opens their full profile preview — the lobby row
  // itself is too compact for a banner surface, so this is how "Match
  // lobby/profile preview" reaches the equipped animated background/header.
  ...
  // Each player row's cosmetics come from p.profile (a PublicProfile from
  // get_public_profiles, now widened to carry all 4 equip columns) — the
  // catalog is the only piece fetched separately, shared across all rows.
        ...
        <div className="glass-card" style={{ padding: '16px 20px', marginBottom: 16 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {players.map((p) => {
              ...
              const eq = p.profile ? resolveCosmetics(catalog, p.profile) : null
              // Ready-state border wins the base look (green/gray is the
              // primary lobby signal); an equipped frame's ring color
              // overrides it, but the ready glow (if any) still layers on.
              const frameStyle = frameAvatarStyle(eq?.frame ?? null, `2px solid ${isReady ? '#10b981' : 'rgba(var(--fg-rgb),0.15)'}`)
              return (
                <div
                  key={p.id}
                  style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '10px 12px', background: 'rgba(var(--fg-rgb),0.04)', borderRadius: 14, border: `1px solid ${mine ? 'rgba(124,58,237,0.3)' : 'rgba(var(--fg-rgb),0.06)'}`, cursor: p.profile ? 'pointer' : 'default' }}
                  onClick={() => { if (p.profile) setPreviewUserId(p.user_id) }}
                >
                  <div style={{ position: 'relative', flexShrink: 0 }}>
                    <Avatar
                      url={p.profile?.avatar_url}
                      size={44}
                      style={{ ...frameStyle, transition: 'border-color 0.3s ease' }}
                    />
                    {eq?.decoration && (
                      <span style={{ position: 'absolute', top: -6, right: isAr ? 'auto' : -6, left: isAr ? -6 : 'auto', fontSize: 14, lineHeight: 1, filter: 'drop-shadow(0 1px 3px rgba(0,0,0,0.4))', pointerEvents: 'none' }}>
                        {eq.decoration.icon}
                      </span>
                    )}
                  </div>
                  <div style={{ flex: 1 }}>
                    ...
                    {eq?.title && (
                      <p style={{ margin: '0 0 2px', fontSize: 10, fontWeight: 700, color: '#9d6fff' }}>
