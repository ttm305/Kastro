...
export default function EmojiDecodeScreen({ onNavigate, lang }: Props) {
  ...
  // ── Room/player fetch failed and we have nothing to show yet — this must
  // never silently render as an empty 0/0 lobby. Distinct from "loading":
  // this is a real, visible error with a retry, not a spinner. ──
  if (!room && fetchError) {
    return (
